...
> 普通用户想不到该交代背景，所以**先引导、给例子**，再据背景提议 level/goal（别冷问）。
- **引导式背景**（给例子）：这项目是干嘛的？（生产/原型demo/内部/学习）· 谁在用、你什么角色？· 最担心哪方面？（越改越乱/逻辑说不清/太慢/看不懂/文档跟不上）
- **据背景提议**：角色/担心 → 提议 `level`；用途 → 提议 `goal`（MVP 略松、长线略严）；让用户确认/改。
- **特殊偏好**：调阈值？有没有"某文件故意很大、别报"（→ suppressions）？
- **归纳 `context`**：把背景压成一句话写进 config，供报告围绕它来讲。
...
context: ""                 # 一句话背景：干嘛的/你的角色/最担心什么；audit·watch 报告据此来讲
