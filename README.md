...
**推荐方式（独立 skill 文件夹，自动发现最稳）**：把本仓库 `audit/`、`fix/`、`setup/`、`watch/` 各装成一个独立 skill——
```
...
```
即把各子目录的内容整体放到对应的 `~/.claude/skills/project-health-<名>/` 下。**仓库里仍保持 `audit/`、`fix/`、`setup/`、`watch/` 这种短目录名，不改结构**——只是安装到本地时各自独立成一个 skill 文件夹。
