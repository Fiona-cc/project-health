...
**推荐方式（独立 skill 文件夹，自动发现最稳）**：把本仓库 `audit/`、`fix/` 各装成一个独立 skill——
```
~/.claude/skills/project-health-audit/     ← 复制自本仓库 audit/（SKILL.md + references/）
~/.claude/skills/project-health-fix/       ← 复制自本仓库 fix/（SKILL.md + references/）
```
即把 `audit/` 的内容整体放到 `~/.claude/skills/project-health-audit/`，把 `fix/` 的内容放到 `~/.claude/skills/project-health-fix/`。**仓库里仍保持 `audit/`、`fix/` 这种短目录名，不改结构**——只是安装到本地时各自独立成一个 skill 文件夹。

**若想保留套件结构**（`project-health/audit/…`、`project-health/fix/…`）：把整个仓库放到 `~/.claude/skills/project-health/`。能否被自动发现，取决于你的 Claude Code 版本对**嵌套 skill** 的支持——发现不到就改用上面的"独立 skill 文件夹"方式。

装好后，新开一个会话：说 **"检查项目健康"** 触发 audit；audit 出报告后说 **"修第 X 项"** 触发 fix。
