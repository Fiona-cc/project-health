...
One-time onboarding for a project. **Detects the project's background (猜后再问), confirms with the user, and writes `.project-health/config.yml`** so `project-health-audit` / `project-health-fix` fit this project. Suggests structure for new projects but **never creates files** (scaffolding is out of scope).
...
3. **Config-only, no scaffolding** — the only output is `.project-health/config.yml`; suggest structure but don't create/rewrite project files.
...
## Config schema (setup is its only writer)
...
verify: "npm run build"     # fix's post-edit safety command (if detected)
